      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {delayedMembers.map((member) => (
            <div
              key={member.id}
              className="p-4 rounded-xl bg-[#13131A] border border-[#262630] hover:border-red-500/50 transition-colors"
            >
              <div className="flex items-start gap-3 mb-3">
                {member.avatar_url ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-[#262630] flex items-center justify-center text-sm font-medium text-[#E8E8ED]">
                    {member.name.charAt(0).toUpperCase()}
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-[#E8E8ED] truncate">
                    {member.name}
                  </h4>
