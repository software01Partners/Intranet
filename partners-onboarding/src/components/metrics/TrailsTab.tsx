...
export function TrailsTab({ trails }: TrailsTabProps) {
  ...
  const deadlineColor: Record<string, string> = {
    ...
    warning: 'text-[#F59E0B]',
    ok: 'text-[#10B981]',
    no_deadline: 'text-[#6B7194] dark:text-[#8888A0]',
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#E2E5F1] dark:border-[#2D2D4A]">
                <th className="text-left py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">
                  <button onClick={() => toggleSort('trailName')} className="flex items-center gap-1 hover:text-[#1A1D2E] dark:hover:text-[#E8E8ED]">
                    Trilha <ArrowUpDown className="w-3 h-3" />
                  </button>
                </th>
                <th className="text-left py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">Tipo</th>
                <th className="text-center py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">Inscritos</th>
                <th className="text-center py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">
                  <button onClick={() => toggleSort('completedCount')} className="flex items-center gap-1 hover:text-[#1A1D2E] dark:hover:text-[#E8E8ED] mx-auto">
                ...
                <th className="text-center py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0] min-w-[140px]">
                  <button onClick={() => toggleSort('averageProgress')} className="flex items-center gap-1 hover:text-[#1A1D2E] dark:hover:text-[#E8E8ED] mx-auto">
                    Progresso <ArrowUpDown className="w-3 h-3" />
                  </button>
                </th>
                <th className="text-center py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">Quiz</th>
                <th className="text-left py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">Prazo</th>
                <th className="text-center py-3 px-2 font-medium text-[#6B7194] dark:text-[#8888A0]">
                  <button onClick={() => toggleSort('overdueCount')} className="flex items-center gap-1 hover:text-[#1A1D2E] dark:hover:text-[#E8E8ED] mx-auto">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-8 text-[#6B7194] dark:text-[#8888A0]">
                ...
                filtered.map((trail) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-3 px-2 font-medium text-[#1A1D2E] dark:text-[#E8E8ED]">
                        {trail.trailName}
                      </td>
